...
#
# Incorporates svn_auth code from andychu-test.googlecode.com/files/googlecode_upload.py
#
# Changes by Luke Pinner 10/06/2011
#     Use win32crypt.CryptUnprotectData to decrypt TortoiseSVN wincrypt password
#     Use hashlib.md5 instead of deprecated md5 module
#
...
def get_svn_auth_via_svn(project_name, config_dir, realm):
  ...
  if auth is not None:
    try:
      if auth['passtype']=='wincrypt':
          import win32crypt
          auth['password']=win32crypt.CryptUnprotectData(base64.decodestring(auth['password']),None, None, None, 0)[1]
...
def get_svn_auth_directly(project_name, config_dir, realm):
  ...
  digest = hashlib.md5(realm).hexdigest()
